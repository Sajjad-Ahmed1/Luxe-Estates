...
export default function VideoCard({ property }) {
  ...
  return (
    <div className="relative group">
      ...
      >
        ...
        <div className="absolute top-3 left-3 flex flex-wrap gap-2 z-10">
          {property.tags.map((tag) => (
            <span
              key={tag}
              className="text-xs font-semibold px-2 py-1 rounded-full bg-black/40 backdrop-blur-sm"
      ...
      {/* Heart — outside Link to prevent navigation */}
      <button
        onClick={(e) => { e.preventDefault(); e.stopPropagation(); toggleWishlist(property.id); }}
        className="absolute top-3 right-3 z-20 w-8 h-8 rounded-full flex items-center justify-center bg-black/40 backdrop-blur-sm transition-transform hover:scale-110"
      ...
      {/* Compare checkbox */}
      <label className="flex items-center gap-2 mt-2 px-1 cursor-pointer w-fit">
        <input
          type="checkbox"
          checked={isCompared}
