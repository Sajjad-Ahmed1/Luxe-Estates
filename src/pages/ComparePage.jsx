...
export default function ComparePage() {
  ...
  const colTemplate = `140px repeat(${compared.length}, minmax(140px, 1fr))`;

  return (
    <motion.div
      className="min-h-screen bg-slate-950 text-white pt-24 md:pt-28 pb-20 px-4 md:px-6"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-wrap items-center justify-between gap-4 mb-6 md:mb-8">
          <div>
            <h1 className="font-display text-3xl md:text-4xl font-semibold">Compare</h1>
        ...
        {compared.length === 0 ? (
          <div className="text-center py-24 text-white/30 text-sm">
            No properties selected. Go back and select up to 2 properties to compare.
          </div>
        ) : (
          /* Horizontal scroll wrapper for mobile */
          <div className="overflow-x-auto -mx-4 md:mx-0 px-4 md:px-0">
            <div className="rounded-2xl border border-white/10 overflow-hidden" style={{ minWidth: 360 }}>

              {/* Header row */}
              <div className="grid border-b border-white/10" style={{ gridTemplateColumns: colTemplate }}>
                <div className="p-3 md:p-4 bg-slate-800/40" />
                {compared.map((p) => (
                  <div key={p.id} className="p-3 md:p-4 bg-slate-800/40 border-l border-white/10">
                    <div className="flex items-start justify-between gap-2">
                      <h3 className="font-semibold text-xs md:text-sm leading-snug">{p.title}</h3>
              ...
              {ROWS.map(({ label, key }) => (
                ...
                >
                  <div className="px-3 md:px-4 py-3 text-xs uppercase tracking-wider text-white/40 bg-slate-900/30">{label}</div>
                  {compared.map((p) => (
                    <div key={p.id} className="px-3 md:px-4 py-3 text-xs md:text-sm text-white/70 border-l border-white/8">
              ...
              {(() => {
                ...
                return allAmenities.map((amenity) => (
                  <div
                    ...
                    style={{ gridTemplateColumns: colTemplate }}
                  >
                    <div className="px-3 md:px-4 py-3 text-xs text-white/40 bg-slate-900/20">{amenity}</div>
                    {compared.map((p) => (
                      <div key={p.id} className="px-3 md:px-4 py-3 border-l border-white/8">
